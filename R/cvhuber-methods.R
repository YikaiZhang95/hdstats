#' Extract Coefficients from a \code{cv.hdhuber} Object
#'
#' Retrieves coefficients from a cross-validated \code{\link{hdhuber}} model,
#' using the stored full-data fit and the value of \code{lambda} selected by
#' cross-validation.
#'
#' @param object A fitted \code{\link{cv.hdhuber}} object.
...
#' @seealso \code{\link{cv.hdhuber}}, \code{\link{predict.cv.hdhuber}}
...
#' y <- x %*% beta_star + eps
#' cv.fit <- cv.hdhuber(x = x, y = y, delta = 0.5, lam2 = 0.01)
#' coef(cv.fit, s = c(0.02, 0.03))
coef.cv.hdhuber <- function(object, s = c("lambda.1se", "lambda.min"), ...) {
  coef(object$hdhuber.fit, s = cv_lambda(object, s), ...)
}

#' Make Predictions from a \code{cv.hdhuber} Object
#'
#' Generates predictions from a cross-validated \code{\link{hdhuber}} model,
#' using the stored full-data fit and the value of \code{lambda} selected by
#' cross-validation.
#'
#' @param object A fitted \code{\link{cv.hdhuber}} object.
...
predict.cv.hdhuber <- function(object, newx, s = c("lambda.1se", "lambda.min"), ...) {
  predict(object$hdhuber.fit, newx, s = cv_lambda(object, s), ...)
}
